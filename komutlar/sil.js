...
exports.run = (Bot, Mesaj, Argüman) => {
    ...
    .setColor("#7289DA")
  {
    if (!Mesaj.member.hasPermission("MANAGE_MESSAGES")) {
      ...
      Hata.setDescription(":negative_squared_cross_mark: **Malesef Yeterli Yetkiniz Bulunmamakta.**");
      Mesaj.channel.send(Hata).then(msg => msg.delete(5000));
      
    } else {
      if (!Sayı) {
        Hata.setDescription(":crescent_moon: **Bir Sayı Belirtiniz.**");
        Mesaj.channel.send(Hata).then(msg => msg.delete(5000));
      } else {
        if (Sayı < 101) {
          Başarılı.setDescription(`:white_check_mark: Başarıyla **${Sayı}** Adet Mesaj Silindi.`);
        ...
        if (Sayı > 1000) {
          Hata.setDescription(":negative_squared_cross_mark: **En fazla 1000 Adet Mesaj Silebilirsiniz.**");
          Mesaj.channel.send(Hata).then(msg => msg.delete(5000));
        }
      }
    }
